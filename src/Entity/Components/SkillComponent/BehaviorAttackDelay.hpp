...
struct BehaviorAttackDelay : AbstractAggregateBehavior {
	...
	void Cast(SkillComponent* comp, std::int32_t behaviorID, RakNet::BitStream* bs) {
		...
		std::int32_t actionToHandle = CacheBehaviorParameter::GetParameterValue(behaviorID, "action");
		std::float_t delay = CacheBehaviorParameter::GetParameterValue(behaviorID, "delay");

		for (int i = 0; i < num_intervals; ++i) {
			std::uint32_t behaviorHandle = actionToHandle;
			bs->Write(behaviorHandle);

			comp->AddBehaviorHandle(behaviorHandle, actionToHandle, delay);
		}
	}
};
#endif