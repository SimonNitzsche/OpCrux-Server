			...
			for (auto playerInv : playerBags) {
				...
				for (auto it = playerItems.begin(); it != playerItems.end(); ++it) {
					...
					if (itemStack.tab == 0 && itemStack.slot >= maxItemSlots) {
						MailManager::SendMail(owner->GetZoneInstance(), owner->GetNameStr(), "MAIL_SYSTEM_NOTIFICATION", "MAIL_INTERNAL_CSR_DEFAULT_SUBJECT", "MAIL_ACTIVITY_OVERFLOW_BODY", false, 0, *it);
	...
	void AddItem(std::int32_t itemLOT, std::uint32_t incCount = 1, DataTypes::Vector3 sourcePos = DataTypes::Vector3(), DataTypes::LWOOBJID iSubKey = 0ULL, LDFCollection metadata = {}, bool subItem = false, uint32_t optionalTab = 0) {
		...
		if (!useStacking) {
			...
			if (tab == 0 && slot > maxItemSlots - 1) {
				...
				// Mail::SendItem(...);
				// return;
				MailManager::SendMail(owner->GetZoneInstance(), owner->GetNameStr(), "MAIL_SYSTEM_NOTIFICATION", "MAIL_INTERNAL_CSR_DEFAULT_SUBJECT", "MAIL_ACTIVITY_OVERFLOW_BODY", false, 0, itemStack.toDBModel());
